...
适用场景：生成 ThinkingData / 数数「留存分析」模型查询请求体，或作为 `使用本 skill 执行` 的 Markdown 任务配置。

## 请求 JSON

请把 留存分析 请求体直接写在下面的 JSON 代码块中。

日期范围通常在 `eventView` 中设置：相对日期用 `recentDay` / `recent_day`，固定日期范围用 `startTime` + `endTime` 或 `from_date` + `to_date`。不要在同一个请求中混用相互冲突的时间范围。

留存天数按“第 N 日留存”口径生成：初始事件当天为第 1 日，N 留对应回访日期为初始日期 + (N - 1)。如果需要转成 SQL 的 `date_diff('day', cohort_date, active_date)`，N 留应使用 `N - 1`，例如 3 留使用 2。
...
[
  {
    "name": "retention-analyze",
    "enabled": true,
    "action": "retention-analyze",
    "mode": "once",
